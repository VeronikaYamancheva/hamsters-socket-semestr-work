...
public class CookieCollectingEventListener extends AbstractEventListener {

    @Override
    public void handle(Message message, int connectionId, int clientsCount) throws EventListenerException {
        if (!isInit) {
            throw new EventListerIsNotInitException(CookieCollectingEventListener.class.getName());
        }
        ...
        server.sendBroadCastMessage(MessageCreator.createMessage(MessageType.COOKIE_COLLECTED_RESPONSE, buffer.array()));
    }

    @Override
    public int getType() {
        return MessageType.COOKIE_COLLECTED_REQUEST;
    }
}
