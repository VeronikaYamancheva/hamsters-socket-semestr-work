...
public class PlayerJumpingEventListener extends AbstractEventListener {
    ...
    public void handle(Message message, int connectionId, int clientsCount) throws EventListenerException {
        ...
        ByteBuffer buffer = ByteBuffer.allocate(INT_BYTES + data.length);
        buffer.putInt(connectionId);
        buffer.put(data);

        server.sendBroadCastMessage(MessageCreator.createMessage(MessageType.PLAYER_JUMP_RESPONSE, buffer.array()));
    }

    @Override
    public int getType() {
        return MessageType.PLAYER_JUMP_REQUEST;
    }
}
