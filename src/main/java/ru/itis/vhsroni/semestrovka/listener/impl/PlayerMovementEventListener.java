...
public class PlayerMovementEventListener extends AbstractEventListener {

    @Override
    public void handle(Message message, int connectionId, int clientsCount) throws EventListenerException {
        if (!isInit) {
            throw new EventListerIsNotInitException(PlayerMovementEventListener.class.getName());
        }
        byte[] data = message.getData();
        ByteBuffer buffer = ByteBuffer.allocate(data.length + INT_BYTES);
        buffer.putInt(connectionId);
        buffer.put(data);
        server.sendBroadCastMessage(MessageCreator.createMessage(MessageType.PLAYER_POSITION_UPDATE_RESPONSE, buffer.array()));
    }

    @Override
    public int getType() {
        return MessageType.PLAYER_POSITION_UPDATE_REQUEST;
    }
}
